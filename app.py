...
from groq_advisor import SUPPORTED_MODELS, test_groq_key, generate_executive_memo, generate_5c_finding_memo, generate_consolidated_master_report
...
if "selected_model" not in st.session_state: st.session_state["selected_model"] = "openai/gpt-oss-20b"
...
with st.sidebar:
    ...
    selected_model = st.selectbox("Inference Model", options=model_options, index=model_options.index(st.session_state["selected_model"]))
...
if uploaded_files:
    # Process files if new uploads are detected
    if len(uploaded_files) != len(st.session_state["multi_file_data"]):
        ...
        for file in uploaded_files:
            ...
            # Run engine silently for the master report
        ...
        # Set the first file as the active view
...
if st.session_state["multi_file_data"]:
    ...
    # Select which file to view interactively
    ...
    if not edited_batch_df.equals(batch_df):
        ...
        # Re-run the engine for this file if edited
        st.session_state["multi_file_data"] = {} # Force reload on edit
        st.rerun()
            ...
            if item["status"] == "FLAGGED":
                ...
                if memo_key in st.session_state["individual_memos"]:
                    ...
                    if st.button(f"Generate 5C Note for Row #{global_row_num}", key=f"btn_5c_{global_row_num}"):
                        note = generate_5c_finding_memo(st.session_state["groq_api_key"], selected_model, {"row_index": global_row_num, "data": current_df.iloc[global_row_num - 1].to_dict(), "flags": item["flags"]}, ALIAS_DEFINITIONS[active_category]['display_name'])
